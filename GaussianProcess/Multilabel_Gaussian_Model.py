...
class LogisticRegression(object):
    ...
    def L2_lost(self, y):
        ...
        return T.mean(T.sqr(y - self.output)/2)

    def negative_log_likelihood(self, y):

        return T.mean(T.sqr((y - self.output)[y.nonzero()]))/2
        #-T.mean((y * T.log(self.output/y))[y.nonzero()])
            #T.mean(T.sqr(y - self.output))/2
               #- 0.01 * T.mean(y * T.log(self.output/y))
        # end-snippet-2
...
def Generate_Set(raw_image_set, size_desired):
    ...
    def class_image(input_image, set_size):
        temp_set = numpy.zeros((set_size, input_image.shape[1], input_image.shape[2]))
        ...
        resultant_set = morphing_coeff[:, None, None] * temp_set[morphing1]+(1-morphing_coeff)[:, None, None]*temp_set[morphing2]
        return resultant_set, morphing1, morphing2
    ...
    generated_image_set, set_order1, set_order2 = class_image(raw_image_set, size_desired)
...
def Generate_Test_Set(raw_image_set, size_desired):
    ...
    def class_image(input_image, set_size):
        ...
        resultant_set = morphing_coeff[:, None, None] * temp_set[morphing1] + morphing_coeff2[:, None, None] * temp_set[morphing2] + morphing_coeff3[:, None, None] * temp_set[morphing3]
    ...
    generated_image_set, set_order = class_image(raw_image_set, size_desired)
    number_of_classes = 10
...
def main_ver1(learning_rate=0.01, weight_decay=0.001, n_epochs=200, nkerns=[20, 30],
    ...
    #layer2_input = T.switch(T.neq(Train_N, 0), T.nnet.bn.batch_normalization(layer2_input_temp, 0.1, 0.1, 0.1, 0.1), layer2_input_temp)
    #theano.tensor.concatenate([layer1.output.flatten(2), layer1a.output.flatten(2)], axis=1)
